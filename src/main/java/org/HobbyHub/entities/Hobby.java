...
@Table(name = "hobby")
@NamedQueries({
        @NamedQuery(name = "Hobby.HobbyUserCountDTO", query = "SELECT new org.HobbyHub.dto.HobbyUserCountDTO(h.id, h.name, CAST(COUNT(u) AS int) ) FROM Hobby h JOIN h.users u GROUP BY h.id"),
})
public class Hobby {
